...
class FirefoxPlugin(ExternalProcess):
    ...
    def start(self):
        ...
        arguments = self.configuration['arguments']
        environment = self.configuration['environment']
        preferences = self.configuration['preferences']
...
class B2G_EmulatorPlugin(ExternalProcess):

    PLUGIN_NAME = 'B2G Emulator'

    def start(self):
        # Launch as sub-plugin because mozbase is not compatible with Python 3.
        b2g_dir = self.configuration['b2g_dir']
        preferences = self.configuration['preferences']
        symbols_path = self.configuration['symbols_path']
        logcat_dir = self.configuration['logcat_dir']
        fuzzer = os.path.basename(self.target)
        cmd = [
            'python2.7', 'utils/firefoxos.py',
            '-b2g_dir', b2g_dir,
            '-symbols_path', symbols_path,
            '-logcat_dir', logcat_dir,
            '-prefs', preferences,
            '-fuzzer', fuzzer,
            '-root', ROOT
        ]
        self.process = self.open(cmd)
